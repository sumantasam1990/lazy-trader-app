...
<template>
    <div>
        <div class="p-2 lg:p-2 bg-white border-b border-gray-200 flex justify-between items-center">
        ...
        <div class="p-4">
            <ul class="bg-white shadow-md rounded-md">
                <li v-for="ticket in tickets.data" :key="ticket.id" class="border-b border-gray-200 bg-white p-2 transition duration-300 transform hover:bg-gray-200 rounded-md">
                    <Link :href="`/ticket/message/${ticket.id}`">
                        <div class="flex justify-between items-center">
                            <div>
                                <p class="text-sm text-gray-500 mb-2">Ticket: {{ticket.ticket_no}}</p>
                                <h2 class="text-md font-bold mb-2">{{ ticket.title }}</h2>
